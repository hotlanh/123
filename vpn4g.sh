...
makeConfig() {
    echo "------  Config 80 & 443 BY Tao ---------"
	read -p "Loại website của bạn: V2board"
	echo "---------------"
	read -p "Link website: https://vpn4g.xyz/"
	echo "---------------"
	read -p "API key của web: khongbietdiencaigibaygio"
	echo "---------------"
  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        CertFile: /etc/XrayR/cert/vpn4g.crt
        KeyFile: /etc/XrayR/cert/vpn4g.key
        Provider: alidns 
        Email: test@me.com
        DNSEnv: 
...
EOF
	cd /etc/XrayR
	git clone https://github.com/hotlanh/cert.git
	XrayR restart
